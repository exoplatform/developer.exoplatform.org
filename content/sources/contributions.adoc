...
* the commit message must start with a keyword 'fix:' or 'feature:' in function of the type of the commine, then a short subject (max 100 characters). It must describes briefly what the commit does.The line must end with the issue references : the github issue (if we are in a meeds-io repository) and the task reference. For example :
----
...
[source,shell]
----
# fix/feature: (If applied, this commit will...) <subject> - EXO-<task-id> - meeds-io/meeds#<github-issue>
...
* If there is a new library or dependency, was it validated by architectes ? What about the library licence ?
* If there is a new visual component, was it validated by Design Guild ?
* Check Performance : check loop, sql queries ... ask if the loop or the query can have a larger number of elements
* Learning
* Unit Tests
* Security
* Troubleshooting
